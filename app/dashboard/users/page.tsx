        ...
        <div className="flex-1 overflow-auto">
          ...
          users.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full py-20 text-center">
              ...
              <p className="text-zinc-400 font-medium text-sm">No end users yet</p>
              <p className="text-zinc-600 text-xs mt-1 max-w-xs">Use your application API key to call <code className="font-mono bg-zinc-800 px-1 rounded">POST /api/users</code> to register users in your application.</p>
            </div>
          ) : (
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-zinc-950 border-b border-zinc-800"><tr>{["User","Application","MFA","Sessions","Last active",""].map(h=>(<th key={h} className="text-left px-4 py-2.5 text-zinc-500 font-medium">{h}</th>))}</tr></thead>
            <tbody>{users.map(u=>(<tr key={u.id} onClick={()=>setSelected(u)} className={`border-b border-zinc-800/40 hover:bg-zinc-900 cursor-pointer ${selected?.id===u.id?"bg-zinc-900":""}`}>
              ...
              <td className="px-4 py-3"><span className="text-indigo-300 text-[11px] font-medium">{u.app_name||<span className="text-zinc-600">—</span>}</span></td>
              <td className="px-4 py-3">{u.mfa_enabled?<ShieldCheck className="w-4 h-4 text-green-400"/>:<ShieldOff className="w-4 h-4 text-zinc-600"/>}</td>
              <td className="px-4 py-3 text-zinc-400">{u.active_sessions??0} <span className="text-zinc-600">/ {u.total_sessions??0}</span></td>
              <td className="px-4 py-3 text-zinc-500 flex items-center gap-1"><Clock className="w-3 h-3"/>{timeAgo(u.last_login_at||u.last_session_at)}</td>
