...
- Name: api/tenant-isolation.spec.ts >> Tenant Isolation — malformed credentials → 401 >> POST /api/risk/evaluate - API key with correct prefix but wrong value → 401
- Location: tests/api/tenant-isolation.spec.ts:70:7
